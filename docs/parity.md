...
- `tui_prompt_queue_during_turn`, `tui_prompt_queue_ctrl_c_pop`: prompts submitted while an agent turn is still running are queued, shown under Vibe's queued-message header, drained FIFO into follow-up model turns, and `Ctrl+C` pops the newest queued prompt before cancelling or quitting.
...
- `tui_bang_empty`, `tui_bang_bash`, `tui_bang_queue_bash`, `tui_bang_large_context`: manual `!` shell input matches Vibe's empty-command error, command-output rendering, saved session context injection, FIFO queuing while a manual bash command is still running, and `bash.max_output_bytes` truncation for large injected stdout/stderr.
